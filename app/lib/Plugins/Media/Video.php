...
class WLPlugMediaVideo extends BaseMediaPlugin implements IWLPlugMedia
{
    ...
    public $opa_media_metadata = array();
    
    public $ops_path_to_ffmeg = null;
    ...
    {
        if (!(bool)$this->getAppConfig()->get("video_preview_generate_frames") && (!isset($pa_options['force']) || !$pa_options['force'])) {
            return false;
        }
        if (!caMediaPluginFFmpegInstalled()) {
            return false;
        ...
        if (($vn_previewed_duration = ($vn_start_at - $vn_end_at)) < 0) {
            ...
            if ($vn_previewed_duration == 0) {
                $vn_previewed_duration = 0.1;
            }
        }
