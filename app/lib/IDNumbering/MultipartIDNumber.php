...
class MultipartIDNumber extends IDNumber
{
    ...
    public function formatHas($ps_element_type, $pn_index=null, $ps_format=null, $ps_type=null, $pa_options=null)
    {
        ...
        if (!is_null($pn_index) && isset($va_elements[$pn_index])) {
            $va_elements = array($va_elements[$pn_index]);
        }
    ...
    public function getNextValue($ps_element_name, $pm_value=null, $pb_dont_mark_value_as_used=false)
    {
            ...
            foreach ($va_elements as $vs_element_name => $va_element_info) {
                switch ($va_element_info['type']) {
                    ...
                    case 'DAY':
                        ...
                        if ($va_element_info['type'] == 'MONTH') {
                            $va_element_vals[]  = $va_date['mon'];
                        }
                        if ($va_element_info['type'] == 'DAY') {
                            $va_element_vals[]  = $va_date['mday'];
                        }
                        break;
                    case 'LIST':
        ...
        $vn_i = 0;
        foreach ($va_elements as $vs_element_name => $va_element_info) {
            if ($vs_element_name == $ps_element_name) {
                break;
            }
    ...
    {
        $vs_separator = $this->getSeparator();
        if (!is_array($va_elements_normal_order = $this->getElements())) {
            $va_elements_normal_order = array();
        }
        $va_element_names_normal_order = array_keys($va_elements_normal_order);

        if (!($va_elements = $this->getElementOrderForSort())) {
            $va_elements = $va_element_names_normal_order;
        }
        ...
        foreach ($va_elements as $vs_element) {
            ...
            switch ($va_element_info['type']) {
                ...
                case 'SERIAL':
                case 'NUMERIC':
                    if ($vn_padding < $va_element_info['width']) {
                        $vn_padding = $va_element_info['width'];
                    }
    ...
    public function getIndexValues($ps_value=null, $pa_options=null)
    {
        ...
        foreach ($va_elements as $vs_element) {
            ...
            if ($vn_max_value_count < sizeof($va_output[$vn_i])) {
                $vn_max_value_count = sizeof($va_output[$vn_i]);
            }
        }
        ...
        if (preg_match_all("![^A-Za-z0-9]+!", $ps_value, $va_delimiters)) {
            ...
            foreach ($va_element_values as $vs_element_value) {
                ...
                if (sizeof($va_delimiters[0]) > 0) {
                    $va_acc[] = array_shift($va_delimiters[0]);
                }
            }
        }
        ...
        switch ($va_element_info['type']) {
            ...
            case 'LIST':
                ...
                if (!$vs_element_value || $va_element_info['editable'] || $pb_generate_for_search_form) {
                    if (!$vs_element_value && !$pb_generate_for_search_form) {
                        $vs_element_value = $va_element_info['default'];
                    }
                    ...
                    foreach ($va_element_info['values'] as $ps_value) {
                        if (trim($ps_value) === trim($vs_element_value)) {
                            $vs_selected = ' selected="selected"';
                        } else {
                            $vs_selected = '';
                        }
            ...
            case 'CONSTANT':
                ...
                if (!$vs_element_value) {
                    $vs_element_value = $va_element_info['value'];
                }
            ...
            case 'DAY':
                ...
                if ($vs_element_value == '') {
                    ...
                    if (!$pb_generate_for_search_form) {
                        if ($va_element_info['type'] == 'YEAR') {
                            $vn_value = ($va_element_info['width'] == 2) ? substr($va_date['year'], 2, 2) : $va_date['year'];
                        }
                        if ($va_element_info['type'] == 'MONTH') {
                            $vn_value = $va_date['mon'];
                        }
                        if ($va_element_info['type'] == 'DAY') {
                            $vn_value = $va_date['mday'];
                        }
                    }
