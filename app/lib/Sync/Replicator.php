        ...
        foreach ($this->getSourcesAsServiceClients() as $vs_source_key => $o_source) {
            ...
            foreach ($this->getTargetsAsServiceClients() as $vs_target_key => $o_target) {
                ...
                while (true) { // use chunks of 10 entries until something happens (success/err)
                    ...
                    if (!is_array($va_source_log_entries)) {
                        break;
                    }
                                                ...
                                                if (sizeof($va_dependent_guids) > 0) {
                                                    ...
                                                    if (is_array($pa_filter_on_access_settings)) {
                                                        ...
                                                        if (is_array($va_access_for_missing)) {
                                                            $missing_guids = array_keys(array_filter($va_access_for_missing, function ($v) use ($pa_filter_on_access_settings) {
                                                                return (($v == '?') || (in_array((int)$v, $pa_filter_on_access_settings, true)));
                                                            }));
                                                            $this->guids_to_skip = array_unique(array_merge($this->guids_to_skip, array_keys(array_filter($va_access_for_missing, function ($v) use ($pa_filter_on_access_settings) {
                    ...
                    foreach ($entry['snapshot'] as $snk => $snv) {
                        if (preg_match("!([A-Za-z0-9_]+)_guid$!", $snk, $matches)) {
                            if (
                                ...
                                if (($matches[1] !== 'lot') && ($matches[1] !== 'lot_id')) {
                                    continue;
                                } 		// TODO: just do dependencies for lots for now; eventually we'll need to consider list items, relationship types, et al.
                                if ($entry['guid'] === $snv) {
                                    continue;
                                }										// don't make item dependency of itself
        ...
        foreach ($source_log_entries_for_missing_guids as $vs_missing_guid => $va_source_log_entries_for_missing_guid) {
            ...
            while (sizeof($va_source_log_entries_for_missing_guid) > 0) {
                ...
                while (sizeof($va_source_log_entries_for_missing_guid) > 0) {
                    ...
                    if ((sizeof($va_entries) >= 10) || (sizeof($va_source_log_entries_for_missing_guid) == 0)) {
                        break;
                    }
                }
                ksort($va_entries);
