 ...
 */
    ...
    {
        if (caGetOption('disableSorts', $pa_options, false)) {
            return [];
        }

        $vs_cache_key = caMakeCacheKeyFromOptions($pa_options, "{$ps_table}/{$pn_type_id}");
        
        if (CompositeCache::contains("available_sorts") && is_array($va_cached_data = CompositeCache::fetch("available_sorts")) && isset($va_cached_data[$vs_cache_key])) {
        ...
        if ($ps_table) {
            ...
            if (caGetOption('distinguishNonUniqueNames', $pa_options, true)) {
                foreach (array_count_values(array_filter($va_base_fields, function ($v) {
                    return !is_null($v);
                })) as $vn_v => $vn_c) {
                    if ($vn_c > 1) {
        ...
        if (($pa_allowed_sorts = caGetOption('allowedSorts', $pa_options, null)) && !is_array($pa_allowed_sorts)) {
        ...
        foreach ($va_base_fields as $vs_k => $vs_v) {
            if (isset($va_ui_bundle_label_map[$vs_k])) {
                ...
                array_pop($va_tmp);
                if (isset($va_ui_bundle_label_map[join('.', $va_tmp)])) {
                    $va_base_fields[$vs_k] = $va_ui_bundle_label_map[join('.', $va_tmp)];
                }
            }
        }
        ...
        if ($pn_display_id > 0) {
            ...
            $va_base_fields = array_filter($va_base_fields, function ($v, $k) use ($va_display_bundles, $config_sorts) {
                ...
                foreach ($va_display_bundles as $b) {
                    if (preg_match("!^{$b}!", $k) || preg_match("!^{$k}!", $b)) {
                        return true;
                    }
                }
                return false;
