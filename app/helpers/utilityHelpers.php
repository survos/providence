...
function _t($ps_key)
{
    ...
    if (isset($g_translation_strings[$ps_key])) {
        return $g_translation_strings[$ps_key];
    }
...
# ----------------------------------------
    ...
    {
        if (caGetOption('useAppTmpDir', $options, false)) {
...
class WebServiceError extends \Exception
{
    ...
    function caValidateCSRFToken($po_request, $ps_token=null, $pa_options=null)
    {
        ...
        if (!$ps_token) {
            $ps_token = $po_request->getParameter('crsfToken', pString);
        }
        if (!is_array($va_tokens = PersistentCache::fetch("csrf_tokens_{$session_id}", "csrf_tokens"))) {
            $va_tokens = [];
        }
